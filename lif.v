`timescale 1ns / 1ps
    ...
    // PARAMETRELER (Q8.12)
    // --------------------------------------------------
    parameter signed [19:0] V_TH         = -20'sd204800; // -50 mV
    parameter signed [19:0] V_RESET      = -20'sd260240; // -65 mV
    parameter signed [19:0] E_EXCITATORY =  20'sd0;      //   0 mV
    parameter signed [19:0] E_INHIBITORY =  -20'sd286720;        //-20'sd368640; // -90 mV Burayı değiştirdim -20'sd368640
    parameter signed [19:0] E_LEAK       = V_RESET;

    // Q0.12
    parameter signed [19:0] dt_tau = 20'sd103; //820

    // Q7.12
    parameter signed [19:0] R_mem  = 20'sd81920; // 20

    parameter [19:0] REFRACTORY_CLOCKS = 20'd12300; // 3ms
...
// --------------------------------------------------
// CONDUCTANCE COMPUTATIONS AND BIT-SHIFTING OPERATIONS
// --------------------------------------------------

    // Excitatory Current
    assign syn_exc_big = R_mem * $signed(i_e_big >>> 12);
    assign syn_exc     = $signed(syn_exc_big >>> 12); 

    // Inhibitory Current
    assign syn_inh_big = R_mem * $signed(i_i_big >>> 12);
    assign syn_inh     = $signed(syn_inh_big >>> 12); 
